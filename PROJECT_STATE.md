...
**`hooks/deny-claude-md-writes.sh` is the mechanism that replaces discipline.** It denies `Edit`/`Write`/`MultiEdit`/`NotebookEdit` on `CLAUDE.md` when the manifest at that root is v4, refuses on cannot-determine, allows on v3/no-manifest, and survives `bypassPermissions`. **Stated non-goal:** Bash writes (`sed -i`, `>>`, heredocs) bypass it, because it matches editing *tools*, not command strings. The backstop is the next sync's `claude_md_identical` FAIL plus overwrite — so a hand-edit is loud and self-healing instead of a permanent silent deviation, which is exactly the failure this project had under the v2 keep-mine model.
