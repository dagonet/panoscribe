...
def require_cuda_for_asr() -> None:
    """Raise :class:`OmniScribeError` if CTranslate2 reports no CUDA device.
    ...
    try:
        ...
        has_cuda = False
    if not has_cuda:
        raise OmniScribeError(f"ASR (CTranslate2): {_REMEDY}")


def require_cuda_for_ocr() -> None:
    """Raise :class:`OmniScribeError` if ONNXRuntime reports no CUDA provider.

    Callers must only invoke this when ``ocr_device == "cuda"``.
    """
    if "CUDAExecutionProvider" not in onnxruntime.get_available_providers():
        raise OmniScribeError(f"OCR (ONNXRuntime): {_REMEDY}")
